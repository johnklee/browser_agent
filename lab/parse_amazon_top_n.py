...
def parse_amazon_new_releases(
  url: str, top_n: int = 10, lang: str = "zh-TW,zh;q=0.9,en-US;q=0.8,en;q=0.7"
):
  headers = {
  ...
  if response.status_code != 200:
    logger.error("Failed to fetch page. HTTP Status: %s", response.status_code)
    return []
...
def save_to_csv(results: list, filename: str):
  ...
  with open(filename, mode="w", newline="", encoding="utf-8-sig") as f:
    ...
    for row in results:
      writer.writerow(row)
  logger.info("Saved %d items to CSV: %s", len(results), filename)


def print_table(results: list):
  print("\n" + "=" * 120)
  print(
    f"{'排名':^6} | {'ASIN':^12} | {'價格':^10} | {'評分 / 評論數':^22} | {'商品名稱'}"
  )
  print("-" * 120)
  for r in results:
    rating_info = (
      f"{r['rating']} ({r['reviews_count']})"
      if r["reviews_count"] != "N/A"
      else r["rating"]
    )
    print(
      f"{r['rank']:^6} | {r['asin']:^12} | {r['price']:^10} | {rating_info:^22} | {r['title']}"
    )
...
def main():
  ...
  parser.add_argument(
    ...
    "-o",
    default=None,
    help="Output CSV file path (default: amazon_top_releases_{category_name}_{date}.csv)",
  )
  parser.add_argument(
    "--sqlite-file",
    "-s",
    default=None,
    help="Path to SQLite DB file to store rank data (e.g. database/rank.db)",
  )
  parser.add_argument(
    "--log-level",
    default="INFO",
